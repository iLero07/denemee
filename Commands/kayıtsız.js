...
exports.run = async (client, message, args) => {
  ...
  const ravi2 = new MessageEmbed().setColor(Settings.Colors.Red).setAuthor(message.author.username, message.author.avatarURL({ dynamic: true }))
...
users.roles.remove(r.id)
  ...
  message.channel.send(embed.setDescription(`${users} Adlı Kullanıcı Başarıyla Kayıtsız'a Atıldı.`).setTimestamp().setColor("RED"))
}
module.exports.conf = {
  enabled: true,
  guildOnly: true,
